...
class DeepQ:
    ...
    def softBackupNetwork(self, model, backup, TAU):
        weightMatrix = []
        for layer in model.layers:
            weights = layer.get_weights()
            weightMatrix.append(weights)
        i = 0
        for layer in backup.layers:
            newWeights = layer.get_weights()
            for j in range(len(weightMatrix[i])):
                newWeights[j] = (1-TAU)*newWeights[j] + TAU*weightMatrix[i][j]
            weights = newWeights
            layer.set_weights(weights)
            i += 1
    ...
    def updateTargetNetwork(self):
        ...
        self.softBackupNetwork(self.model, self.targetModel,0.1)
    ...
    # calculate the target function
    def calculateDDQNTarget(self, NewState, qValuesNewState, reward, isFinal):
        """
        target = reward(s,a) + gamma * targetQ(argmax(Q(s')
        """
        if isFinal:
            return reward
        else :
            return reward + self.discountFactor * qValuesNewState.flatten()[np.argmax(self.model.predict(NewState.reshape(1,len(NewState))))]
    ...
    def selectActionByProbability(self, qValues, bias): # ================== TODO: what is this bias?
        qValueSum = 0
        shiftBy = 0
        for value in qValues: # ================== TODO: is this piece of code trying to make all Qs non-negative?
            if value + shiftBy < 0:
        ...
        qValueProbabilities[len(qValueProbabilities) - 1] = 1 # ================== TODO: why is it cumulative?
    ...
    def learnOnMiniBatch(self, miniBatchSize, useTargetNetwork=True):
        ...
        if self.memory.getCurrentSize() > self.learnStart:
            ...
            for sample in miniBatch:
                ...
                targetValue = self.calculateDDQNTarget(newState, qValuesNewState, reward, isFinal)
    ...
    def loadWeights(self, path):
        ...
        # weights_list = load_model(path).get_weights()
