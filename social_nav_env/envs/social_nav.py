...
class SocialNav(gym.Env):
    ...
    def configure(self, config):
        ...
        # Map related config
        self.map = Map(self.config.map_width, self.config.map_height, self.config.map_type, []) #TODO
    ...
    def laser_scan(self, agent):
        laser_scan_data = []
        divisions = self.scan_num_lasers - 1
        min_ang = self.scan_ang_range[0]
        max_ang = self.scan_ang_range[1]
        min_dist = self.scan_dist_range[0]
        max_dist = self.scan_dist_range[1]

        for i in range(self.scan_num_lasers): # For each laser angle
            scan_ang = agent.theta + min_ang + i * (max_ang - min_ang) / divisions
            # Extend laser checkpoint outwards until collison is detected
            for scan_reach in np.linspace(min_dist, max_dist, self.scan_num_dist_checks):
                scan_end_x = agent.px + math.floor(scan_reach * math.cos(scan_ang))
                scan_end_y = agent.py + math.floor(scan_reach * math.sin(scan_ang))
                if not self.map.contains_position((scan_end_x, scan_end_y)) or self.map.has_obstacle_at((scan_end_x, scan_end_y)):
                    break
            self.map.draw_line((agent.px, agent.py), (scan_end_x, scan_end_y), self.scan_colour)
